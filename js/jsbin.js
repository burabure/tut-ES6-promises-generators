var jsBin = (function (window, document, undefined) {
  ...
  function embed(jsBinSection) {
    var iframe = document.createElement('iframe'),
        resize = document.createElement('div'),
        url = jsBinSection.getAttribute("data-jsbin").replace(/edit/, 'embed');
    iframe.src = url.split('&')[0];
    iframe._src = url.split('&')[0]; // support for google slide embed
    iframe.className = jsBinSection.className; // inherit all the classes from the link
    iframe.id = jsBinSection.id; // also inherit, giving more style control to the user
    iframe.style.border = '1px solid #aaa';

    var query = getQuery(url.replace(/.*(\?.*)/, "$1"));
    iframe.style.width = query.width || '100%';
    iframe.style.minHeight = query.height || '300px';
    jsBinSection.appendChild(iframe);
